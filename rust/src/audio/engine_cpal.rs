...
#[cfg(not(target_os = "android"))]
use super::buffer_pool::{BufferPoolChannels, AudioThreadChannels};
#[cfg(not(target_os = "android"))]
...
impl AudioEngine {
    ...
    fn create_input_stream(&self, mut channels: AudioThreadChannels) -> Result<cpal::Stream, AudioError> {
        ...
        let stream = match config.sample_format() {
            ...
            _ => return Err(AudioError::StreamOpenFailed { 
                reason: "Only F32 sample format is currently supported for input".to_string() 
            }),
        }.map_err(|e| AudioError::StreamOpenFailed { reason: format!("{:?}", e) })?;
    ...
    pub fn start(
        ...
        input_stream.play().map_err(|e| AudioError::HardwareError { details: format!("Input start failed: {}", e) })?;
        output_stream.play().map_err(|e| AudioError::HardwareError { details: format!("Output start failed: {}", e) })?;
