...
void main() {
  group('TrainingScreen', () {
    ...
    testWidgets('displays Start button when not training',
      ...
      expect(find.widgetWithText(FloatingActionButton, 'Start'), findsOneWidget);
    ...
    group('permission handling', () {
      testWidgets('requests permission before starting audio',
        ...
        when(() => mockPermissionService.checkMicrophonePermission())
            .thenAnswer((_) async => PermissionStatus.granted);
        when(() => mockAudioService.startAudio(bpm: any(named: 'bpm')))
            .thenAnswer((_) async {});
        when(() => mockAudioService.getClassificationStream())
        ...
        verify(() => mockPermissionService.checkMicrophonePermission())
            .called(1);
      });
      ...
      testWidgets('shows permission denied dialog when permission denied',
        ...
        verify(() => mockPermissionService.requestMicrophonePermission())
            .called(1);
      ...
      testWidgets('opens app settings when Open Settings button tapped',
        ...
        when(() => mockPermissionService.checkMicrophonePermission())
            .thenAnswer((_) async => PermissionStatus.permanentlyDenied);
        when(() => mockPermissionService.openAppSettings())
    ...
    group('BPM updates during training', () {
      testWidgets('updates BPM in real-time when slider moved during training',
          (WidgetTester tester) async {
        // Setup: permission granted, audio started
        when(() => mockPermissionService.checkMicrophonePermission())
            .thenAnswer((_) async => PermissionStatus.granted);
        when(() => mockAudioService.startAudio(bpm: any(named: 'bpm')))
            .thenAnswer((_) async {});
        when(() => mockAudioService.setBpm(bpm: any(named: 'bpm')))
            .thenAnswer((_) async {});
        when(() => mockAudioService.getClassificationStream())
            .thenAnswer((_) => Stream.value(
                  const ClassificationResult(
        ...
        verify(() => mockAudioService.setBpm(bpm: any(named: 'bpm')))
